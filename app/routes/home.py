...
from database.models import Message, User, Chatroom
...
def get_all_messages( authorization : str = Header(...), db: Session = Depends(get_db)):
    ...
    group_results = (
        ...
        .join(subq, (Message.room_id == subq.c.room_id) & (Message.sent_at == subq.c.latest_sent_at))
        .filter(Message.sender_id == userinfo.id)
        .all()
    )
    ...
    def get_timestamp(entry):
        if entry["type"] == "personal":
            return datetime.strptime(entry["last_message"]["timestamp"], "%Y-%m-%d %H:%M:%S.%f")
